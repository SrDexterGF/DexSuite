...
public sealed class M04DeepCleanup : ModuleExecutorBase
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        if (Want(enabledSubOps, "M04_crash_dumps"))
        {
            yield return Step("Crash dumps y volcados de memoria");
            long dumpBytes = 0;
            dumpBytes += PurgeFile("C:\\WIN386.SWP");
            var (mdf, mdb) = PurgeDirectory(Path.Combine(windir, "Minidump"), ct);
            dumpBytes += PurgeFile(Path.Combine(windir, "memory.dmp"));
            var (cdf, cdb) = PurgeDirectory(Path.Combine(localApp, "CrashDumps"), ct);
            totalFiles += mdf + cdf;
            dumpBytes += mdb + cdb;
            totalBytes += dumpBytes;
            yield return Ok($"Crash dumps eliminados ({FormatBytes(dumpBytes)})");
        }

        if (ct.IsCancellationRequested) yield break;
        if (Want(enabledSubOps, "M04_spooler"))
        {
        ...
        if (ct.IsCancellationRequested) yield break;
        if (Want(enabledSubOps, "M04_event_log"))
        {
